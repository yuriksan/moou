...
// Re-evaluates automatically whenever load() returns fresh data.
...
// Auto-close panel once everything is in sync
...
function parseUrl(url: string): { entityType: string; entityId: string } {
  ...
  // ValueEdge: /ui/entity-navigation?p=...&entityType=work_item&id=12345
  try {
    const parsed = new URL(url);
    if (parsed.pathname.includes('entity-navigation')) {
      const id = parsed.searchParams.get('id');
      if (id) return { entityType: 'work_item', entityId: id };
    }
  } catch {
...
<template>
  <aside class="detail-panel">
    ...
    <template v-else-if="outcome">
      ...
      <div v-if="showSyncPanel" class="sync-preview-panel">
        ...
        <div v-if="titleOutOfSync" class="sync-field-row">
          ...
          <div class="sync-preview-body">
            <div class="sync-value">
              <div class="sync-value-label">moou (current)</div>
              <div class="sync-value-text">{{ outcome.title }}</div>
            </div>
            <div class="sync-value sync-value-remote">
              <div class="sync-value-label">{{ backendProviderLabel || 'remote' }}</div>
              <div class="sync-value-text">{{ primaryCache?.title || '(empty)' }}</div>
            </div>
          </div>
        ...
        <div v-if="descriptionOutOfSync" class="sync-field-row">
          ...
          <div class="sync-preview-body">
            <div class="sync-value">
              <div class="sync-value-label">moou (current)</div>
              <div class="sync-value-text">{{ outcome.description || '(empty)' }}</div>
            </div>
            <div class="sync-value sync-value-remote">
              <div class="sync-value-label">{{ backendProviderLabel || 'remote' }}</div>
              <div class="sync-value-text">{{ (primaryCache?.description as string) || '(empty)' }}</div>
            </div>
          </div>
          <div class="sync-field-actions">
      ...
      <!-- No local description but VE has one — show a hint so user can pull it -->
      <div v-else-if="!editing && descriptionOutOfSync" class="description-sync-hint">
        <span>No description — {{ backendProviderLabel || 'remote' }} has one</span>
...
.sync-preview-body { display: grid; grid-template-columns: 1fr 1fr; gap: 0; }
